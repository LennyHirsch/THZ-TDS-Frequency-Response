...
def E1(n_thz, X2, omega, t_pp):
    return( ( (E0**2 * X2 * t_pp * np.sqrt(np.pi)) * np.exp(- (t_pp**2 * omega**2) / 4) ) / (2*(n_thz**2 - n_g**2)))
...
def E(n_thz, X2, omega, z, t_pump):
    return( E1(n_thz, X2, omega, t_pump)*(E2(n_thz, omega, z) + E3(n_thz, omega, z) - E4(omega, z)) )
...
def eps_thz(omega):
    return(eps_inf + ( ((eps_0 - eps_inf) * omega_TO**2)/(omega_TO**2 - omega**2 + 1j*gamma_TO*omega) ))
